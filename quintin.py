...
class Ui_MainWindow(QtWidgets.QWidget):
	...
	def takeinputs(self):
		name, done1 = QtWidgets.QInputDialog.getText(
			self, 'Input Dialog', 'Enter The country:')

		roll, done2 = QtWidgets.QInputDialog.getInt(
		self, 'Input Dialog', 'Enter PIO:')

		cgpa, done3 = QtWidgets.QInputDialog.getDouble(
			self, 'Input Dialog', 'Enter Shop:')

		country =['Kenya', 'Uganda', 'Tanzania', 'Sudan', 'Rwanda']
		countr, done4 = QtWidgets.QInputDialog.getItem(
		self, 'Input Dialog', 'countruage you know:', country)
		...
		if done1 and done2 and done3 and done4 :
			...
			self.label.setText('Information stored Successfully\nName: '
								+str(name)+'('+str(roll)+')'+'\n'+'CGPA: '
								+str(cgpa)+'\nSelected countruage: '+str(countr))
